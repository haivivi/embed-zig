...
main() {
    ...
    if [ -n "$ESP_WIFI_SSID" ]; then
        CMAKE_ARGS="$CMAKE_ARGS -DCONFIG_WIFI_SSID=$ESP_WIFI_SSID"
        echo "[esp_build] WiFi SSID: $ESP_WIFI_SSID"
    fi
    if [ -n "$ESP_WIFI_PASSWORD" ]; then
        CMAKE_ARGS="$CMAKE_ARGS -DCONFIG_WIFI_PASSWORD=$ESP_WIFI_PASSWORD"
    fi
    if [ -n "$ESP_TEST_SERVER_IP" ]; then
        CMAKE_ARGS="$CMAKE_ARGS -DCONFIG_TEST_SERVER_IP=$ESP_TEST_SERVER_IP"
