...
function(esp_zig_build)
    ...
    # zig build-lib with Bazel-generated module args (no build.zig)
    #
    # Args files (generated by Bazel esp_zig_app rule):
    #   zig_module_args.txt  — -M, --dep, -I flags for all modules
    #   zig_lib_a_args.txt   — .a files from deps for C symbol linking
    #   zig_main_args.txt    — main module --dep + -M flags
    #
    # ESP-IDF include dirs are added as global -I flags so @cImport
    # in any module can find sdkconfig.h, freertos headers, etc.
    add_custom_target(zig_build
        WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
        COMMAND ${CMAKE_COMMAND} -E env "ESP_INCLUDE_DIRS=${ESP_INCLUDE_DIRS}"
        bash -c "
            ZIG=$ENV{ZIG_INSTALL}/zig
            
            # Build -I flags from ESP-IDF include dirs
            ESP_I=''
            IFS=';' read -ra DIRS <<< \"$$ESP_INCLUDE_DIRS\"
            for d in \"$${DIRS[@]}\"; do
                [ -n \"$$d\" ] && ESP_I=\"$$ESP_I -I $$d\"
            done
            ...
            echo '[zig] build-lib target=${ZIG_TARGET} cpu=${TARGET_CPU_MODEL}'
            $$ZIG build-lib \
                -lc \
                $$ESP_I \
                $$MAIN_ARGS \
