...
pub fn Client(comptime Socket: type) type {
    const socket = trait.socket.Socket(Socket);  // 编译时验证
    return struct {
        // ... 使用 socket 类型
    };
}
...
// I2C 接口验证
pub fn Driver(comptime I2c: type) type {
    trait.i2c.is(I2c);  // 编译时验证
    return struct {
        // ... 使用 I2c 类型
    };
}
...
SDK 模块导入 `trait` 进行接口验证，运行时使用平台特定实现。
