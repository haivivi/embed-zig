//! Korvo-2 V3 Board Implementation for AEC Test
//!
//! Uses esp.idf module wrappers for ESP-IDF APIs.
...
// ============================================================================
// C Helper Functions (from lib/esp)
// ============================================================================
...
fn es8311Write(reg: u8, val: u8) void {
    ...
    _ = i2c_helper_write(Hardware.es8311_addr, &buf, 2, 100);
}

fn es8311Read(reg: u8) u8 {
    var val: [1]u8 = undefined;
    _ = i2c_helper_write_read(Hardware.es8311_addr, &[_]u8{reg}, 1, &val, 1, 100);
    return val[0];
}

fn es7210Write(reg: u8, val: u8) void {
    const buf = [_]u8{ reg, val };
    _ = i2c_helper_write(Hardware.es7210_addr, &buf, 2, 100);
}

fn es7210Read(reg: u8) u8 {
    var val: [1]u8 = undefined;
    _ = i2c_helper_write_read(Hardware.es7210_addr, &[_]u8{reg}, 1, &val, 1, 100);
    return val[0];
}
...
// ============================================================================
// Codec Init (exact copy from working aec_test_zig)
// ============================================================================
...
fn initAudioSystem() !void {
    ...
    std.log.info("AudioSystem: Init I2C", .{});
    if (i2c_helper_init(Hardware.i2c_sda, Hardware.i2c_scl, 400000, 0) != 0) {
        return error.I2cInitFailed;
    }

    std.log.info("AudioSystem: Init ES8311", .{});
    es8311Init();

    std.log.info("AudioSystem: Init ES7210", .{});
    es7210Init();

    std.log.info("AudioSystem: Init I2S", .{});
    if (i2s_helper_init_std_duplex(Hardware.i2s_port, Hardware.sample_rate, 32, Hardware.i2s_bclk, Hardware.i2s_ws, Hardware.i2s_din, Hardware.i2s_dout, Hardware.i2s_mclk) != 0) {
        return error.I2sInitFailed;
    }
    _ = i2s_helper_enable_rx(Hardware.i2s_port);
    _ = i2s_helper_enable_tx(Hardware.i2s_port);

    std.log.info("AudioSystem: Start ES8311", .{});
    es8311Start();
    es8311SetVolume(150);

    idf.time.sleepMs(10);

    std.log.info("AudioSystem: Start ES7210", .{});
    es7210Start();

    std.log.info("AudioSystem: Init AEC", .{});
...
fn deinitAudioSystem() void {
    ...
    g_aec_output = null;
    g_tx_buffer_32 = null;
    _ = i2s_helper_deinit(Hardware.i2s_port);
    i2c_helper_deinit();
    g_initialized = false;
}
...
// ============================================================================
// PA Switch Driver (using idf.gpio)
// ============================================================================
...
pub const SpeakerDriver = struct {
    ...
    pub fn write(_: *Self, buffer: []const i16) !usize {
        ...
        _ = i2s_helper_write(Hardware.i2s_port, tx_bytes.ptr, tx_bytes.len, &bytes_written, 1000);
...
pub const MicDriver = struct {
    ...
    pub fn read(_: *Self, buffer: []i16) !usize {
        ...
        const ret = i2s_helper_read(Hardware.i2s_port, raw_bytes.ptr, to_read, &bytes_read, 1000);
