...
//!
//! Two layers:
//! - **Resampler**: thin SpeexDSP wrapper, single-batch `process([]i16, []i16)`
...
// ============================================================================
// Resampler (SpeexDSP thin wrapper)
// ============================================================================
...
pub const Resampler = struct {
    ...
    pub fn process(self: *Resampler, in_buf: []const i16, out_buf: []i16) !Result {
        var in_frames: u32 = @intCast(in_buf.len / self.channels);
        var out_frames: u32 = @intCast(out_buf.len / self.channels);
        try self.inner.processInterleaved(
            in_buf.ptr,
            &in_frames,
            out_buf.ptr,
            &out_frames,
        ...
        return .{
            .in_consumed = in_frames * self.channels,
            .out_produced = out_frames * self.channels,
        };
    }
...
pub fn StreamResampler(comptime Rt: type) type {
    ...
    return struct {
        ...
        // With stereo expansion (2x) and 6x upsampling, each chunk can
        // produce up to max_chunk_frames * 2 * 6 = 768 output i16 values.
        const max_chunk_frames = 128;
