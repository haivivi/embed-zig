...
//!
//!     [group_id: u24][index: u5][count: u5][parity: 1][_: 1][payload_len: u16]
//!
//! Simplified to a byte-aligned layout:
//!
//!     [group_id: u16][index: u8][count: u8][payload_len: u16][payload...]
//!
...
/// XOR src into dst (dst ^= src). Buffers must be equal length.
...
pub const Encoder = struct {
    ...
    /// Using a flat array to avoid allocator dependency.
...
pub const Decoder = struct {
    ...
    fn tryRecover(self: *Decoder, group: *Group) void {
        ...
        // The recovered payload length is unknown exactly, but the parity_len
        // is the max payload length in the group. We stored individual lengths
        // but don't have the missing one. Use parity_len as upper bound.
        // For KCP, the receiver will validate via KCP's own length fields.
...
test "FEC encoder/decoder roundtrip with no loss" {
    const TestCtx = struct {
        var received: [16][]const u8 = undefined;
