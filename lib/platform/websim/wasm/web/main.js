...
// ============================================================================
// Screen Recorder (html2canvas captures device div → canvas → webm → mp4)
// ============================================================================

// Screen recorder: html2canvas → canvas → webm → ffmpeg → mp4
// States: idle → recording → converting
let recState = 'idle';
...
const deviceFrame = document.querySelector('.sim-container');
const recCanvas = document.createElement('canvas');

if (screenRecBtn && deviceFrame) {
    screenRecBtn.addEventListener('click', () => {
        if (recState === 'converting') {
            // Cancel conversion
            convertAborted = true;
            if (window._ffmpeg) {
                try { window._ffmpeg.terminate(); window._ffmpeg = null; } catch(_) {}
            }
            return;
        }
        if (recState === 'recording') {
            ...
            deviceFrame.classList.remove('recording');
            ...
            // Start
            recState = 'recording';
            deviceFrame.classList.add('recording');
            recordedChunks = [];
            ...
            mediaRecorder.onstop = async () => {
                ...
                screenRecBtn.textContent = '\u2716 Converting...';
                const webmBlob = new Blob(recordedChunks, { type: 'video/webm' });
                const ts = new Date().toISOString().slice(0,19).replace(/:/g,'-');

                try {
                    if (!window._ffmpeg) {
                        screenRecBtn.textContent = '\u2716 Loading FFmpeg...';
                    ...
                    if (convertAborted) throw new Error('Cancelled');

                    const ffmpeg = window._ffmpeg;
                    let dotCount = 0;
                    const dotTimer = setInterval(() => {
                        dotCount = (dotCount + 1) % 4;
                        screenRecBtn.textContent = '\u2716 Converting' + '.'.repeat(dotCount);
                    }, 500);

                    await ffmpeg.writeFile('input.webm', new Uint8Array(await webmBlob.arrayBuffer()));
                    if (convertAborted) throw new Error('Cancelled');

                    await ffmpeg.exec(['-i', 'input.webm', '-c:v', 'libx264', '-preset', 'fast', '-crf', '23', '-pix_fmt', 'yuv420p', 'output.mp4']);
                    ...
                    a.href = url;
                    a.download = 'websim-' + ts + '.mp4';
                    a.click();
                    URL.revokeObjectURL(url);
                    screenRecBtn.textContent = 'Saved!';
                } catch(e) {
                    try { clearInterval(dotTimer); } catch(_) {}
                    if (convertAborted) {
                        screenRecBtn.textContent = 'Cancelled';
                    } else {
                        console.error('MP4 failed:', e);
                        screenRecBtn.textContent = 'Error!';
                        const ep = document.getElementById('errorPanel');
                        const ec = document.getElementById('errorContent');
                        if (ep && ec) {
                            ep.style.display = '';
                            ec.innerHTML += '<div class="log-line error">MP4: ' + e + '</div>';
                        }
                    }
                }
                recState = 'idle';
                setTimeout(() => { screenRecBtn.textContent = '\u25CF Rec'; }, 2000);
            };

            // Screenshot loop: html2canvas creates new canvas each frame,
            // then we drawImage onto recCanvas (reusing canvas directly causes black frames)
            ...
            recTimerId = setInterval(() => {
                if (typeof html2canvas !== 'undefined') {
                    html2canvas(deviceFrame, {
                        ...
                        recCtx.drawImage(shot, 0, 0, recCanvas.width, recCanvas.height);
                    }).catch(() => {});
                }
            }, 50);

            mediaRecorder.start(100);
            screenRecBtn.textContent = '\u25A0 Stop';
            screenRecBtn.classList.add('recording');
        }
    });
}
...
const logAutoScrollBtn = document.getElementById('logAutoScroll');
if (logAutoScrollBtn) {
    logAutoScrollBtn.classList.add('active');
    logAutoScrollBtn.addEventListener('click', () => {
        logAutoScroll = !logAutoScroll;
        logAutoScrollBtn.classList.toggle('active', logAutoScroll);
        logAutoScrollBtn.textContent = logAutoScroll ? '\u21A7 Auto' : '\u21A7 Off';
    });
}
