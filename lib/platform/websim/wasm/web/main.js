...
// Diamond layout: 1-2-3-2-1 = 9 LEDs
const DIAMOND_ROWS = [1, 2, 3, 2, 1];
let ledElements = [];
let ledsBuilt = false;
...
function buildLEDs() {
    ...
    ledElements = [];
    let idx = 0;
    for (const count of DIAMOND_ROWS) {
...
function updateLEDs() {
    ...
    if (!ledsBuilt && ledCount > 0) buildLEDs();

    for (let i = 0; i < ledElements.length && i < ledCount; i++) {
