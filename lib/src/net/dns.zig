//! DNS Resolver with interface binding and UDP/TCP support
...
pub const DnsResolver = struct {
    ...
    /// Network interface to bind (null = auto)
    ...
    pub fn resolve(self: *const Self, hostname: []const u8) !Ipv4Address {
    ...
    fn resolveUdp(self: *const Self, hostname: []const u8) !Ipv4Address {
        var sock = socket.Socket.udp() catch return error.SocketError;
        defer sock.close();

        if (self.interface) |iface| {
            sock.bindToDevice(iface) catch return error.SocketError;
        }
        sock.setRecvTimeout(self.timeout_ms);

        // Build query
        var query_buf: [512]u8 = undefined;
        const query_len = buildQuery(&query_buf, hostname, 0x1234) catch return error.QueryBuildFailed;
        ...
        const response_len = sock.recvFrom(&response_buf) catch |err| {
            if (err == socket.SocketError.Timeout) return error.Timeout;
    ...
    fn resolveTcp(self: *const Self, hostname: []const u8) !Ipv4Address {
        var sock = socket.Socket.tcp() catch return error.SocketError;
        defer sock.close();
        ...
        const query_len = buildQuery(query_buf[2..], hostname, 0x1234) catch return error.QueryBuildFailed;
...
fn buildQuery(buf: []u8, hostname: []const u8, transaction_id: u16) !usize {
...
fn parseResponse(data: []const u8) !Ipv4Address {
