...
/// Usage:
/// ```zig
/// const TlsStream = tls.Stream(Socket, Rng, allocator);
/// const HttpClient = http.ClientWithTls(Socket, TlsStream);
/// ```
pub fn Stream(comptime Socket: type, comptime Rng: type, comptime allocator: std.mem.Allocator) type {
    return struct {
        client: tls_client.Client(Socket, Rng),
        socket: Socket,
        hostname: []const u8,
        ...
        pub fn init(socket: Socket, options: Options) !Self {
            ...
            const config = tls_client.Config{
                .allocator = allocator,
                .hostname = "",
            ...
            self.client = try tls_client.Client(Socket, Rng).init(&self.socket, config);
            return self;
        }
...
pub fn StreamWithAllocator(comptime Socket: type, comptime Rng: type) type {
    return struct {
        client: ?tls_client.Client(Socket, Rng),
        socket: *Socket,
        ...
        pub fn init(socket: *Socket, alloc: std.mem.Allocator, options: Options) !Self {
            ...
            self.client = try tls_client.Client(Socket, Rng).init(socket, config);
            return self;
        }
